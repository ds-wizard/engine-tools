...
def create_pot_file(ctx, template_dir, output, force: bool):
    ...
    load_local(tdk, template_dir)
    try:
        pot_file = tdk.create_pot_file(output=pathlib.Path(output), force=force)
    except Exception as e:
        ClickPrinter.failure('Failed to create the POT file')
        ClickPrinter.error(f'> {e}')
        sys.exit(1)
    for failed_file in pot_file.failed_files:
        ClickPrinter.warning(f'Skipped file {failed_file} that could not be parsed')
    filename = click.style(output, bold=True)
