...
class TDKCore:
    ...
    def _check_metamodel_version(self):
        ...
        version_tag = f'v{version_tuple[0]}.{version_tuple[1]}.{version_tuple[2]}'
        ...
        if min_version > version_tuple:
            raise TDKProcessingError(f'Unsupported metamodel version for API {version_tag}', hint)
    ...
    async def load_remote(self, template_id: str):
        ...
        for template_file in files:
            self.safe_template.template_file[template_file.filename.as_posix()] = template_file
        ...
        for template_file in assets:
            self.safe_template.files[template_file.filename.as_posix()] = template_file
    ...
    async def _update_template_file(self, remote_file: TemplateFile, local_file: TemplateFile,
        ...
        try:
            ...
            try:
                ...
                await self._delete_template_file(template_file=remote_file)
                await self._create_template_file(template_file=local_file, project_update=True)
    ...
    async def _delete_template_file(self, template_file: TemplateFile, project_update: bool = False):
        try:
            self.logger.debug('Deleting existing remote %s %s (%s) started',
                              template_file.remote_type.value, template_file.filename.as_posix(),
                              template_file.remote_id)
            if template_file.remote_type == TemplateFileType.ASSET:
                result = await self.safe_client.delete_template_draft_asset(
                    remote_id=self.remote_id,
                    asset_id=template_file.remote_id,
                )
            else:
                result = await self.safe_client.delete_template_draft_file(
                    remote_id=self.remote_id,
                    file_id=template_file.remote_id,
                )
            self.logger.debug('Deleting existing remote %s %s (%s) finished: %s',
                              template_file.remote_type.value, template_file.filename.as_posix(),
                              template_file.remote_id, 'ok' if result else 'failed')
            if project_update and result:
                self.safe_project.remove_template_file(template_file.filename)
        except Exception as e:
            self.logger.error('Failed to delete existing remote %s %s: %s',
                              template_file.remote_type.value, template_file.filename.as_posix(), e)

    async def cleanup_remote_files(self, remote_assets: list[TemplateFile],
                                   remote_files: list[TemplateFile]):
        for template_file in self.safe_project.safe_template.files.values():
            self.logger.debug('Cleaning up remote %s', template_file.filename.as_posix())
            for remote_asset in remote_assets:
                if remote_asset.filename == template_file.filename:
                    await self._delete_template_file(template_file=remote_asset, project_update=False)
            for remote_file in remote_files:
                if remote_file.filename == template_file.filename:
                    await self._delete_template_file(template_file=remote_file, project_update=False)

    async def _create_template_file(self, template_file: TemplateFile, project_update: bool = False):
        try:
            ...
            self.logger.debug('Storing remote %s %s finished: %s',
                              template_file.remote_type.value, template_file.filename.as_posix(), result.remote_id)
    ...
    async def store_remote_files(self):
        ...
        for template_file in self.safe_project.safe_template.files.values():
            template_file.remote_id = None
            template_file.remote_type = TemplateFileType.FILE if template_file.is_text else TemplateFileType.ASSET
            await self._create_template_file(template_file=template_file, project_update=True)
    ...
    def create_package(self, output: pathlib.Path, force: bool):
        ...
        with zipfile.ZipFile(output, mode='w', compression=zipfile.ZIP_DEFLATED) as pkg:
            ...
            assets = []
            for template_file in self.safe_project.safe_template.files.values():
                if template_file.is_text:
                    self.logger.info('Adding template file %s', template_file.filename.as_posix())
                    files.append({
                        'uuid': str(UUIDGen.generate()),
                        'content': template_file.content.decode(encoding=DEFAULT_ENCODING),
                        'fileName': str(template_file.filename.as_posix()),
                    })
                else:
                    self.logger.info('Adding template asset %s', template_file.filename.as_posix())
                    assets.append({
                        'uuid': str(UUIDGen.generate()),
                        'contentType': template_file.content_type,
                        'fileName': str(template_file.filename.as_posix()),
                    })
                    self.logger.debug('Packaging template asset %s', template_file.filename.as_posix())
                    pkg.writestr(f'template/assets/{template_file.filename.as_posix()}', template_file.content)
    ...
    async def delete_file(self, filepath: pathlib.Path):
        if not filepath.is_file():
            ...
            return
        try:
            template_file = self.safe_project.get_template_file(filepath=filepath)
            if template_file is None:
                self.logger.info('File %s not tracked currently - skipping',
                                 filepath.as_posix())
                return
            await self._delete_template_file(template_file=template_file, project_update=True)
        except Exception as e:
