...
- Fixed rendering template variables in mail subject
...
- Support for custom mail templates via S3
- Added question title to assigned comment notification email
...
- Added `emailChange` mail template for email change verification
...
- Adapted to change of DB model of locales (ID to UUID)
...
- Strip trailing `/` from client URL in context (passed as `ctx` to templates)
...
- Fixed missing `clientUrl` in mail templates
