...
from dsw.templating.pot import extract_messages
...
@pytest.fixture
def step(make_template, tmp_path: pathlib.Path) -> Jinja2Step:
    root = tmp_path / ROOT_FILE
    root.parent.mkdir(parents=True, exist_ok=True)
    root.write_text(SOURCE, encoding='utf-8')
    return Jinja2Step(make_template(tmp_path), {'template': ROOT_FILE})
...
def test_reindenting_the_template_keeps_the_msgid():
