...
from dsw.templating.http import RequestsWrapper
from dsw.templating.settings import RequestsSettings, SecuritySettings
from dsw.templating.urls import UrlNotAllowedError, UrlPolicy
...
def make_wrapper(*, limit=100, **security) -> RequestsWrapper:
    ...
    return RequestsWrapper(
        settings=RequestsSettings(enabled=True, limit=limit, timeout=1),
        policy=UrlPolicy(SecuritySettings(**options)),
    )
